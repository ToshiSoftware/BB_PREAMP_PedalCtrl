...
void ReadHC165(void) {
  ...
  hc165.data = 0;
  // serial read
  digitalWrite(SRD_CS, HIGH);  // put CS up,  HC165 latches the data
  ...
  digitalWrite(SRD_CS, LOW);  // put CS down
  // Decode
  HandleEncoder();
  // rotate 7seg
  if(appVars.available_wifi)
  {
    Rotate7Seg();
  }
}
...
/* -------------------------------------------------------------------
<how to handle tact SWs to recognize short or long press>
                         a      b    c  d    e              f
                    -------------------->2sec----------------------
...
// step value check
int ParamRangeCheckAD5263( int param )
{
...
// Serial output, 7seg-LED, HC595 and AD5263
void SendMyHC595Serial(int mute)
{
  unsigned char seg7=0, hc595=0;
  int step_volume, step_gain, step_treble, step_bass;
  ...
  // AD8405 step
  step_volume   = 255*appParam.volume/99;
  step_gain =     255*appParam.gain/99;
  step_treble =   255*(appParam.treble+50)/100;
  step_bass =     255*(appParam.bass+50)/100;

  step_volume = ParamRangeCheckAD5263(step_volume);
  step_gain = ParamRangeCheckAD5263(step_gain);
  step_treble = ParamRangeCheckAD5263(step_treble);
  step_bass = ParamRangeCheckAD5263(step_bass);
